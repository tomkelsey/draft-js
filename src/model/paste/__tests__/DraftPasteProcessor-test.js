...
describe('DraftPasteProcessor', function() {
  ...
  it('must identify italics text', function() {
    ...
    var {contentBlocks: output} = DraftPasteProcessor.processHTML(html, CUSTOM_BLOCK_MAP, INLINE_STYLES);
    var block = output[0];
  ...
  it('must preserve spaces', function() {
    ...
    output = DraftPasteProcessor.processHTML(html, CUSTOM_BLOCK_MAP, INLINE_STYLES);
  ...
  it('Should detect when somthing is un-styled in a child', function() {
    ...
    let output = DraftPasteProcessor.processHTML(html, CUSTOM_BLOCK_MAP, INLINE_STYLES);
